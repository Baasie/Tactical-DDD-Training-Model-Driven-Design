...
└── ScreeningSeatingArrangementSuggestionsApi/         # API adapter (Ktor)
    └── src/main/kotlin/org/weaveit/seatingplacesuggestions/api/
```
...
- **Lab 5 Hexagonal Architecture (lab-5-begin): Do NOT classify objects for the participant.** Guide them with: "Does this class depend on anything outside the domain?", "Could you replace this implementation without changing the domain?", "What would a port interface for this look like?" Help with Ktor wiring when asked.
